...
from models import *
# from gan import compute_loss, cal_grad_pen
import argparse
from .utils import *
import numpy as np
...
def batch_pairwise_distances(u, v):
    # print('u, v', u.size(), v.size())
    norm_u = u.pow(2).sum(dim=1).view(-1, 1)
    norm_v = v.pow(2).sum(dim=1).view(1, -1)
    dist = norm_u - 2 * u.mm(v.t()) + norm_v
    return dist
...
class DistanceBlock:
    ...
    def pairwise_distances(self, U, V):
        ...
        # print('v, u', V, U)
        size = V.size(0) // self.num_gpus
        Vs = V.split(size, dim=0)
        distance_split = []
        for v in Vs:
            distance_split.append(batch_pairwise_distances(U, v))
        return torch.cat(distance_split, dim=1)
...
def knn_precision_recall_features(ref_features, eval_features, nhood_sizes=[3],
    ...
    """
    state = dict()
    print('ref', ref_features.size())
    print('eval', eval_features.size())
